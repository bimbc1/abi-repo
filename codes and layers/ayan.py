...
DEFAULT_EXPECTED_INTERVAL_SECONDS = 86400  # 24h default
MIN_SAMPLES_BEFORE_LEARNING = 5            # learn only after N observations
...
def put_metric(namespace, metric_name, value, unit="Count", dimensions=None):
    """
    Publish a CloudWatch custom metric directly via the API, the same way
    the partner Lambda does it. No log line, no metric filter, no
    CloudFormation dependency in the middle -- this call either succeeds
    (metric is recorded immediately) or raises/logs a warning if it fails.
    """
    try:
        metric_data = {
